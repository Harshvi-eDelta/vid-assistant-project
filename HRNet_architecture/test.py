...
def heatmap_to_landmarks(heatmaps, img_size=256, heatmap_size=64):
    """
    Converts heatmaps (64x64) to (x, y) coordinates in the original image scale.
    """
    upsampled_heatmaps = F.interpolate(heatmaps, size=(img_size, img_size), mode='bilinear', align_corners=False)
    landmarks = []
    for heatmap in upsampled_heatmaps.squeeze(0).detach().cpu().numpy():
        y, x = np.unravel_index(np.argmax(heatmap), heatmap.shape)
        landmarks.append((x, y))  # (x, y) in image coordinates
